...
// Mirrors the offence options from Report.jsx
const OFFENCE_TYPES = [
...
export default function ReportScreen({ navigation }) {
  ...
  // Form fields — same shape as Report.jsx payload
  ...
  // ── Fetch regions from GET /regions ────────────────────────────────────────
  useEffect(() => {
    (async () => {
      try {
        ...
        Alert.alert('Network Error', 'Could not load regions. Make sure the backend is running and API_BASE_URL is correct.');
      } finally {
  ...
  // ── Fetch locations whenever region changes (GET /locations?region=) ───────
  useEffect(() => {
    if (!region) return;
    (async () => {
      try {
  ...
  // ── Submit: upload photo → POST /api/reports ───────────────────────────────
  async function handleSubmit() {
    if (!validate()) return;
    setSubmitting(true);
    try {
      // Step 1 — upload photo to POST /api/upload if attached
      let imageUrl = null;
      if (imageUri) {
      ...
      // Step 2 — POST report payload (same shape as Report.jsx web version)
      const payload = {
        region,
        location,
      ...
      // Step 3 — success: show reference ID then navigate home
      Alert.alert(
        '✓ Report Submitted!',
      ...
      >
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>
            Report illegal poaching or wildlife offences. Your report goes to wildlife officers.
          </Text>
        </View>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.formGroup}>
            ...
            {loadingRegions ? (
              ...
              <View style={[styles.pickerWrap, errors.region && styles.fieldError]}>
                ...
                >
                  {regions.map((r) => (
                    <Picker.Item key={r} label={r} value={r} color="#ffffff" />
                  ))}
                </Picker>
              </View>
            )}
          ...
          <View style={styles.formGroup}>
            <Text style={styles.label}>
              📍 General Location <Text style={styles.required}>*</Text>
            </Text>
            <View style={[styles.pickerWrap, errors.location && styles.fieldError]}>
              ...
              >
                {locations.map((l) => (
                  <Picker.Item key={l} label={l} value={l} color="#ffffff" />
                ))}
              </Picker>
            </View>
          ...
          <View style={styles.formGroup}>
            ...
            <Text style={styles.hintText}>
              Tap below to auto-fill your current coordinates.
            </Text>
            {coordinates ? (
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerWrap}>
              ...
              >
                {OFFENCE_TYPES.map((o) => (
                  <Picker.Item key={o.value} label={o.label} value={o.value} color="#ffffff" />
                ))}
              </Picker>
            </View>
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.dateRow}>
              ...
              >
                <Text style={styles.dateBtnText}>
                  {when ? when.toLocaleDateString() : 'Pick Date'}
                </Text>
              </TouchableOpacity>
              ...
              >
                <Text style={styles.dateBtnText}>
                  {when ? when.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Pick Time'}
                </Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={styles.nowBtn}
                onPress={() => setWhen(new Date())}
              >
            ...
            {when && (
              <TouchableOpacity onPress={() => setWhen(null)}>
                <Text style={[styles.removeLink, { marginTop: 4 }]}>Clear date</Text>
              </TouchableOpacity>
            )}
          </View>
